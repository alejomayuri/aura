...
export default function RegisterPage() {
  ...
  const [checkingAuth, setCheckingAuth] = useState(true); // <-- Estado para evitar el parpadeo
  const router = useRouter();

  // Redirigir si el usuario ya está autenticado y ocultar la vista mientras verifica
  useEffect(() => {
  ...
  // Si está comprobando la sesión, devolvemos un contenedor vacío para evitar ver el formulario parpadear
  if (checkingAuth) {
    ...
    <div className="flex min-h-screen w-full bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between pt-6 pb-8 px-8 sm:px-12 lg:px-16 relative z-10 overflow-y-auto">
        ...
        <div className="w-full max-w-md mx-auto my-auto py-8">
          <div className="mb-8">
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">Únete a Lightjaus</h2>
            <p className="text-sm sm:text-base text-slate-600 mt-2 font-normal">Regístrate gratis</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2">Confirmar contraseña</label>
              <input 
                type="password" 
          ...
          <div className="mt-6 text-center text-sm text-slate-600 font-normal">
            ...
            <Link href="/admin/login" className="text-purple-900 font-semibold hover:underline">
              Inicia sesión
            </Link>
          </div>
        </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-purple-950 via-purple-900 to-indigo-950 relative items-center justify-center p-12 overflow-hidden">
        ...
        <div className="relative z-10 w-full h-full max-w-xl max-h-[800px] rounded-3xl border border-purple-500/20 shadow-2xl overflow-hidden flex items-center justify-center bg-purple-950/50 backdrop-blur-md">
          <img 
            ...
            alt="Preview Register Lightjaus" 
